...
import { wbToCategory } from "../wb-categories.cjs";
...
// ============================================================
// Фильтр не-сумок (одежда и т.п.)
// ============================================================

/** Ключевые слова не-сумок — такие товары не попадают в каталог Moranti */
const NON_BAG_KEYWORDS = [
  "футболк", "толстовк", "свитшот", "лонгслив", "худи", "брюк", "шорт",
  "плать", "юбк", "куртк", "пальт", "костюм", "одежд", "обувь", "кроссовк",
  "ботинк", "сапог", "туфл", "носк", "колготк", "бельё", "белье", "пижам",
  "шапк", "шарф", "перчатк", "ремень", "ремен", "галстук", "головной убор",
  "кошелёк", "кошелек", "визитниц", "обложк", "чехол",
];

/**
 * Проверяет, является ли карточка WB сумкой (а не одеждой/аксессуаром).
 * Смотрит subjectName и title карточки.
 *
 * @param {object} card — карточка WB (Content API или card.json)
 * @returns {boolean} true — сумка, false — не сумка (футболка и т.п.)
 */
export function isBagProduct(card) {
  if (!card) return true;
  const haystack = [
    card.subjectName,
    card.subject_name,
    ...
    .join(" ")
    .toLowerCase();
  return !NON_BAG_KEYWORDS.some((kw) => haystack.includes(kw));
}
