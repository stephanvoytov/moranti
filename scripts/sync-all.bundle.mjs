...
var BASE_URL = "https://seller-analytics-api.wildberries.ru";
...
async function wbFetchStocks(_nmIDs, apiKey, log2 = noopLog3) {
  if (!apiKey) return /* @__PURE__ */ new Map();
  ...
  const periodEnd = now.toISOString().split("T")[0];
  log2.write("  Fetching WB stocks (full report, 90d)...");
  try {
    const resp = await fetch(`${BASE_URL}/api/v2/stocks-report/products/products`, {
      ...
      signal: AbortSignal.timeout(FETCH_TIMEOUT3)
    });
    if (resp.status === 429) {
      log2.line(`
  ...
  \u26A0\uFE0F Stocks API ${resp.status}: ${errText.slice(0, 200)}`);
    ...
    log2.line(` \u2014 ${stockMap.size} \u0442\u043E\u0432\u0430\u0440\u043E\u0432 (\u0441\u043E \u0441\u0442\u043E\u043A\u043E\u043C: ${inStock}, \u0431\u0435\u0437: ${zeroStock})`);
  } catch (err) {
    log2.line(`
