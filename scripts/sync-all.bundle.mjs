...
function translitRu(s) {
  if (!s) return "";
  return String(s).toLowerCase().split("").map((ch) => RU_TO_LAT[ch] ?? ch).join("");
}
function buildUrlSlug(name, colorName) {
  const parts = [name, colorName].map((p) => (p || "").trim()).filter(Boolean);
  if (parts.length === 0) return null;
  const slug = translitRu(parts.join(" ")).replace(/[^a-z0-9\s-]+/g, " ").replace(/[\s_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80).replace(/-+$/g, "");
  return slug || null;
}
...
function mergeProductSources(wbCard, wbPrices, wbRating, ozonInfo, ozonAttrs, ozonRating, db) {
  ...
  const urlSlug = buildUrlSlug(finalName, finalColor);
    ...
    try {
      ...
      for (const p of products) {
        ...
        cardMap.set(p.id, {
          ...
          // остаток (0 если нет)
          rating: (p.reviewRating ?? p.rating) || null,
          // дробный рейтинг (reviewRating) — тот, что на витрине; fallback на целый rating
