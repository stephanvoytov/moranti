...
export function mergeProductSources(wbCard, wbPrices, wbRating, ozonInfo, ozonAttrs, ozonRating, db) {
  ...
  // Display price = min across AVAILABLE marketplaces (stock > 0).
  ...
  if (wbCard && (db?.nameAutoGenerated !== false)) {
    const newName = generateName({
      ...
      colorName: data.colorName || db?.colorName || null,
    });
    if (newName !== (db?.name || "")) {
      data.name = newName;
  ...
  // ─── Не-сумки (футболки и т.п.) → архив, не в каталог ───
  if (wbCard && !isBagProduct(wbCard) && !db?.archivedAt) {
    data.archivedAt = new Date();
  }

  // ─── URL slug: из названия + цвета (не из внутреннего артикула) ───
  // Считаем из ФИНАЛЬНЫХ значений (с учётом уже добавленных в data изменений).
  // Уникальность гарантируется в sync-all.mjs (ensureUniqueSlug).
  const finalName = data.name ?? db?.name ?? "";
  const finalColor = data.colorName ?? db?.colorName ?? "";
  // Авто-имя уже содержит цвет — не дублируем его в слаге
  const autoName = (data.nameAutoGenerated ?? db?.nameAutoGenerated) === true;
  const urlSlug = buildUrlSlug(finalName, autoName ? "" : finalColor);
  if (urlSlug && urlSlug !== (db?.slug ?? "")) {
    data.slug = urlSlug;
  }
