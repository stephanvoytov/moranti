...
async function syncToDb(searchMap, cardMap, dbProducts, syncLog) {
  ...
  for (const [article, sp] of searchMap) {
    ...
    // Upgrade image URLs (c516x688 → c1200x1600) or new photos
    if (sp.pics != null) {
      const currentCount = db.images ? db.images.length : 0;
      const isOldSize = db.image && db.image.includes("c516x688");
      if (sp.pics > currentCount || isOldSize) {
        const newImages = cardCdnUrls(article, sp.pics);
        priceUpdates.images = newImages;
        if (newImages.length > 0) priceUpdates.image = newImages[0];
      }
    }
    ...
    if (cardArticles.has(article)) {
      ...
      // Upgrade image URLs from c516x688 → c1200x1600
      if (db.image && db.image.includes("c516x688")) {
        const photoCount = sp?.pics || card?.media?.photo_count || db.images?.length || 1;
        cardUpdates.image = cardCdnUrl(article, 1);
        cardUpdates.images = cardCdnUrls(article, photoCount);
      }
